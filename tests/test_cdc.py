...
def test_data_chunks_empty():
    assert list(cdc.data_chunks(b"", utf32=False)) == [b""]


def test_data_chunks_1byte():
    assert list(cdc.data_chunks(b"\x00", utf32=False)) == [b"\x00"]


def test_data_chunks_below_min():
    data = static_bytes(256 - 1)
    assert list(cdc.data_chunks(data, utf32=False)) == [data]
...
def test_data_chunks_two_chunks():
    ...
    assert list(cdc.data_chunks(data, utf32=False)) == [data[:-1], data[-1:]]
...
def test_data_chunks_max_odd():
    ...
    hashes = [blake3(c).hexdigest() for c in cdc.data_chunks(data, utf32=False)]
    assert len(hashes) == 7
...
def test_data_chunks_amount():
    with open("file_image_lenna.jpg", "rb") as infile:
        chunks2 = list(cdc.data_chunks(infile.read(), utf32=False))
